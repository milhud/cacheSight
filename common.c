...
// Add these helper functions to verify enum values
const char* access_pattern_to_string_debug(access_pattern_t pattern) {
    LOG_DEBUG("access_pattern_to_string called with enum value: %d", pattern);
    const char* result = access_pattern_to_string(pattern);
    LOG_DEBUG("Returning string: %s", result);
    return result;
}
