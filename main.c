...
static int run_static_analysis(const analysis_config_t *config,
    ...
    // After ast_analyzer_analyze_file returns
    LOG_INFO("Static analysis complete: %d patterns, %d loops, %d structs", 
            results->pattern_count, results->loop_count, results->struct_count);

    // ADD THIS DETAILED PATTERN DUMP:
    // ADD THIS DETAILED PATTERN DUMP:
    LOG_DEBUG("=== DUMPING ALL STATIC PATTERNS ===");
    for (int i = 0; i < results->pattern_count; i++) {
        static_pattern_t *p = &results->patterns[i];
        LOG_DEBUG("Pattern %d: %s:%d - %s (array: %s, var: %s, stride: %d, pointer: %s)",
                i, p->location.file, p->location.line,
                access_pattern_to_string(p->pattern),
                p->array_name, p->variable_name, p->stride,
                p->is_pointer_access ? "YES" : "NO");
    }
    LOG_DEBUG("=== END PATTERN DUMP ===\n");
...
static miss_type_t classify_miss_type(cache_hotspot_t *hotspot, cache_info_t *cache_info) {
    if (hotspot->miss_rate > 0.8) {
        return MISS_COMPULSORY;
    } else if (hotspot->miss_rate > 0.5) {
        return MISS_CAPACITY;
    } else if (hotspot->miss_rate > 0.3) {
...
static int determine_affected_levels(cache_hotspot_t *hotspot, cache_info_t *cache_info) {
    int levels = 0;
    for (int i = 0; i < 4; i++) {
    ...
    return levels ? levels : 0x07; // Default to L1,L2,L3 if no data
}

static double calculate_performance_impact(classified_pattern_t *pattern, cache_info_t *cache_info) {
    // Simple impact calculation based on miss rate and latency
    if (pattern->hotspot) {
        double impact = pattern->hotspot->miss_rate * 100.0;
        if (pattern->hotspot->avg_latency_cycles > 100) {
            impact *= 1.5;
        }
        return impact > 90.0 ? 90.0 : impact;
    }
    return 50.0; // Default
}

static void generate_pattern_description(classified_pattern_t *pattern) {
    snprintf(pattern->description, sizeof(pattern->description),
             "%s pattern detected with %.1f%% miss rate",
             cache_antipattern_to_string(pattern->type),
             pattern->hotspot ? pattern->hotspot->miss_rate * 100 : 0);
...
static int run_analysis(const analysis_config_t *config) {
    ...
    // In run_analysis, improve synthetic pattern generation
    if (sample_count == 0 && static_results.pattern_count > 0) {
        ...
        hotspot_count = static_results.pattern_count;
            ...
            switch(sp->pattern) {
                ...
                case STRIDED:
                    if (sp->stride > 8) {
                        ...
                        hs->miss_rate = 0.5;
                    } else if (sp->stride == 8 || sp->stride == 16) {
                        pat->type = BANK_CONFLICTS;
                        hs->miss_rate = 0.4;
                    } else {
                        pat->type = UNCOALESCED_ACCESS;
                        hs->miss_rate = 0.3;
                    }
                    hs->avg_latency_cycles = 12 + sp->stride * 2;
                    break;
                ...
                case ACCESS_LOOP_CARRIED_DEP:
                    pat->type = CACHE_LOOP_CARRIED_DEP;
                    hs->miss_rate = 0.4;
                    hs->avg_latency_cycles = 50;
                    break;
                ...
                case INDIRECT_ACCESS:
                    pat->type = UNCOALESCED_ACCESS;
                    hs->miss_rate = 0.6;
                    hs->avg_latency_cycles = 80;
                    break;
            }
            
            // Set working set size estimate
            hs->address_range_start = 0x1000000;
            hs->address_range_end = hs->address_range_start + 
                (sp->estimated_footprint > 0 ? sp->estimated_footprint : 1024 * 1024);
            
            // Set pattern-specific fields
            pat->severity_score = hs->miss_rate * 100;
            pat->confidence = 0.7;  // Lower for synthetic
            pat->performance_impact = calculate_performance_impact(pat, &cache_info);
            pat->primary_miss_type = classify_miss_type(hs, &cache_info);
            pat->affected_cache_levels = determine_affected_levels(hs, &cache_info);
            
            // Generate detailed descriptions
            generate_pattern_description(pat);
        }
        
        pattern_count = hotspot_count;

        // Where static patterns are converted to classified patterns
        LOG_DEBUG("=== CONVERTING STATIC TO CLASSIFIED PATTERNS ===");
        LOG_DEBUG("=== CONVERTING STATIC TO CLASSIFIED PATTERNS ===");
        for (int i = 0; i < static_results.pattern_count; i++) {
            static_pattern_t *sp = &static_results.patterns[i];
            
            // Before creating hotspot
            LOG_DEBUG("Converting pattern %d: %s:%d", i, sp->location.file, sp->location.line);
            LOG_DEBUG("  Static pattern type: %s (enum: %d)", 
                    access_pattern_to_string(sp->pattern), sp->pattern);
            
            // After creating hotspot
            LOG_DEBUG("  Hotspot dominant_pattern set to: %s (enum: %d)",
                    access_pattern_to_string(hotspots[i].dominant_pattern),
                    hotspots[i].dominant_pattern);
            
            // After classification
            LOG_DEBUG("  Classified as: %s", 
                    cache_antipattern_to_string(patterns[i].type));
        }
        LOG_DEBUG("=== END CONVERSION ===\n");
    }
    ...
    if (!config->no_recommendations && pattern_count > 0) {
        ...
        if (engine) {
            ...
            // SAVE TO FILE
            recommendation_engine_save_to_file(recommendations, rec_count,
                                            "recommendations.txt");
