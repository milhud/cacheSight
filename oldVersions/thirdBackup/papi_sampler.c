...
/// Fallback for systems without sched_getcpu
#ifndef HAVE_SCHED_GETCPU
#include <errno.h>
// Only define if not available
...
static int sched_getcpu(void) {
    ...
    return 0;  // Default to CPU 0 on error
}
#endif
#endif

// Define gettid if not available
#ifndef gettid
...
static int init_papi(void) {
    ...
    if (version != PAPI_VER_CURRENT) {
        ...
        pthread_mutex_unlock(&g_papi_init_mutex);
        return PAPI_VERSION; //papi version
    }
...
papi_sampler_t* papi_sampler_create(const papi_config_t *config) {
    ...
    if (config->overflow_threshold > 0 && config->num_events > 0) {
        int ret = PAPI_overflow(sampler->event_set, sampler->event_codes[0],
                       1000000, 0, papi_overflow_handler); // SAMPLING_THRESHOLD should be defined but will pass 1 million as example
        if (ret != PAPI_OK) {
