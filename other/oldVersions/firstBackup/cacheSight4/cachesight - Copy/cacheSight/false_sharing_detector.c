...
int detect_false_sharing(const cache_miss_sample_t *samples, int sample_count,
    ...
    for (int i = 0; i < sample_count; i++) {
        ...
        while (info && info->cache_line != cache_line) {
            prev = info;
            info = info->samples;  // Reusing pointer as next
        }
    ...
    // Sort candidates by contention score
    if (candidate_count > 0) {
        qsort(candidates, candidate_count, sizeof(false_sharing_candidate_t),
              [](const void *a, const void *b) {
                  const false_sharing_candidate_t *ca = (const false_sharing_candidate_t *)a;
                  const false_sharing_candidate_t *cb = (const false_sharing_candidate_t *)b;
                  if (ca->contention_score > cb->contention_score) return -1;
                  if (ca->contention_score < cb->contention_score) return 1;
                  return 0;
              });
    }
